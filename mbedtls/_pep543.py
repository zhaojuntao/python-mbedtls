...
# ServerNameCallback = Callable[
#     [TLSBufferObject, Optional[str], TLSConfiguration], Any]
ServerNameCallback = Any
...
class TLSWrappedSocket:
    ...
    def negotiated_protocol(self):  # -> Optional[Union[NextProtocol, bytes]]
        """
        Returns the protocol that was selected during the TLS handshake.
        This selection may have been made using ALPN, NPN, or some future negotiation mechanism.
...
class PrivateKey(metaclass=ABCMeta):
    ...
    def from_buffer(cls,
                    buffer: bytes,
                    password: Optional[Union[Callable[[], Union[
                        bytes, bytearray]], bytes, bytearray]] = None):
        """
...
class TrustStore(metaclass=ABCMeta):
    ...
    def from_pem_file(cls,
                      path: Union[pathlib.Path, bytes, str]):  # -> TrustStore
        """
